...
- [x] Проверить что редирект на карту
- [x] Проверить что профиль создан в Table Editor → `users`

### 7. Проверка функциональности

- [x] Открыть приложение в другом браузере / инкогнито (второй аккаунт)
- [x] Нажать "Share Location"
- [x] Проверить что маркер появился на карте
- [x] Нажать на маркер → открывается чат
- [ ] Отправить сообщение
