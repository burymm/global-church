...
export const useLocationStore = create<LocationState>((set, get) => ({
  ...
  fetchOnlineLocations: async () => {
    try {
      ...
      const { data } = await supabase
        .from('users')
        .select('id, display_name, display_icon, avatar_url, denomination, statuses, location_lat, location_lng, is_sharing_location, updated_at, is_online, auth_name')
      ...
      const locations: UserLocation[] = (data || [])
        ...
        .map((u) => ({
          ...
          is_online: u.is_online, auth_name: u.auth_name,
        }));
  ...
  startHeartbeat: () => {
    ...
    get().handleHeartbeat();
    const interval = setInterval(get().handleHeartbeat, 60000);
  ...
  startSharing: () => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      async (pos) => {
        if (!get().isSharing || get().isUpdating) return;
        set({ isUpdating: true });
        try {
          ...
          await get().fetchOnlineLocations();
        } finally {
